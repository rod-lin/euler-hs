...
-- naive search
data FullTree
    = Node Integer FullTree FullTree
    | Leaf Integer
    deriving (Show)

readData' :: [[Integer]] -> [FullTree] -> FullTree
readData' [] [root] = root
readData' [] lst = error "no root present"
readData' (l:ls) [] = readData' ls (map Leaf l)
readData' (l:ls) nodes = readData' ls $ for (zip [0..] l) $ \(i, v) ->
    Node v (nodes !! i) (nodes !! (i + 1))

readData :: [[Integer]] -> FullTree
readData dat = readData' (reverse dat) []

maxRoute :: FullTree -> Integer
maxRoute (Node v left right) = v + max (maxRoute left) (maxRoute right)
maxRoute (Leaf v) = v

-- a faster version
reduce :: [Integer] -> [Integer]
reduce [] = error "empty list"
reduce [n] = [n]
reduce lst = map (uncurry max) (zip (init lst) (tail lst))

reduceTree :: [[Integer]] -> Integer
reduceTree tree =
    let fold layer res =
            if null res then reduce layer
            else reduce (map (uncurry (+)) (zip layer res))
    in head (foldr fold [] tree)

-- dat1 1074
-- dat2 7273
solve = (reduceTree dat1, reduceTree dat2)
